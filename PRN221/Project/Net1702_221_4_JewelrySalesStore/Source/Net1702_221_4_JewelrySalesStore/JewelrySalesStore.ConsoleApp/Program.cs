    ...
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            */
            ...
            var companyList = companyBusiness.GetAll();

            if (companyList.Status > 0 && companyList.Result.Data != null)
            {
                var companies = (List<Company>)companyList.Result.Data;
            ...
            {
                Console.WriteLine("Get All Companies fail");
            }

            Console.WriteLine("DAO.GetById(code)");
            var companyResult = companyBusiness.GetById("040d52a4-c728-40d3-b6d6-286c8a1057cf");

            if (companyResult.Status > 0 && companyResult.Result.Data != null)
            {
                var company = (Company)companyResult.Result.Data;
                Console.WriteLine(company.CompanyName);
            }

            Console.WriteLine("DAO.Save(code)");
            companyResult = companyBusiness.GetById("6c0424ef-7d98-4dea-85d0-8ba97db4f13d");
            if (companyResult.Status > 0 && companyResult.Result.Data != null)
            {
                Console.WriteLine("This company is exist");
            }
            else
            {
                var item = new Company()
                {
                    CompanyId = Guid.Parse("6c0424ef-7d98-4dea-85d0-8ba97db4f13d"),
                    CompanyName = "SpringPrecious",
                    CompanyAddress = "Vinhomes Grand Park",
                    CompanyDescription = "A premier provider of fine jewelry and luxury items.",
                    CompanyPhone = "0123-456-789"               
                };

                var result = companyBusiness.Save(item);
                Console.WriteLine(result.Result.Message);
            }
